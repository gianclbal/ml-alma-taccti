...
def cultural_capital_identification(input_path, output_path, thematic_code):
    ...
    * Essay: Why I am here?
    ...
    human_annotation_social_headers = ["Human Theme Presence (Yes or No)", "Human Annotated Essay", "Specific Theme(s)"]
    # Write the headers to columns K, L, and M
    worksheet.write('K1', human_annotation_social_headers[0])
    worksheet.write('L1', human_annotation_social_headers[1])
    worksheet.write('M1', human_annotation_social_headers[2])
    ...
    del result, writer, workbook, worksheet, marked_essays, csv_modify, csv_read
    gc.collect()

    return out_file_name # removed result as a output, because we dont need it as its processed by the worksheet 
@profile 
...
if __name__ == "__main__":
    ...
    for theme_key, theme_val in themes.items():
        ...
        for i in filtered_input_dir:
            ...
            out_file_name = f"{output_path}/" + str(csv_name) + f"_{themes[3]}_tacited.xlsx"
            print("/Users/gbaldonado/Developer/ml-alma-taccti/ml-alma-taccti/src/tacit/Social" + out_file_name)
            # print("HEY HERE", os.path.isfile(out_file_name))
            # print("HEY HERE", out_file_name)
            if overwrite == "False" and os.path.isfile("/Users/gbaldonado/Developer/ml-alma-taccti/ml-alma-taccti/src/tacit/Social" + out_file_name):
